// legend.js — centralized visual key: color scale & encodings
import { state, metrics } from './state.js';
import { paletteInterpolator, metricDomain, CAT_COLORS } from './colors.js';
...
export function createLegend(sel) {
  ...
  root.html(`
    <div class="legend-grid">
      ...
      <div class="legend-block">
        <div class="legend-title">How dots work</div>
        <div class="legend-swatches" id="lg-cats"></div>
        <div class="legend-caption">Dots on the scatter chart = <b>one state</b>, averaged over the selected window. Click to trace it everywhere.</div>
      </div>
    </div>
  `);
  updateLegend(state);
